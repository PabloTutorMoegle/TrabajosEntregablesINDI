...
uniform vec3 lightPos; // Posición del foco de luz (sol)
uniform vec3 lightColor; // Color de la luz del foco
...
void main()
{
    ...
    // Calcular componentes del spotlight 2
    ...
    // Sumar contribuciones de los focos
    vec3 result = ambient + diffuse + specular + spotDiffuse1 + spotSpecular1 + spotDiffuse2 + spotSpecular2;

    // Calcular iluminación de las antorchas si están encendidas
    for (int i = 0; i < 6; ++i) {
        if (torchOn[i] == true)
        {
